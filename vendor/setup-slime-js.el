...
(setq slime-js-target-url "http://circle.dev")
...
(add-hook 'coffee-mode-hook (lambda () (slime-js-minor-mode 1)))
...
(defun slime-js-coffee-eval-buffer ()
  ...
  (slime-js-eval-buffer)
  (kill-buffer coffee-compiled-buffer-name)
  (slime-js-eval "$('.modal').find('.close').click(); mytop = $('body').scrollTop(); window.VM = new CI.inner.CircleViewModel(); SammyApp.refresh(); $('body').scrollTop(mytop)") ;; make this a hook?
  )
...
(defun slime-js-haml-coffee-eval-buffer ()
  ...
  (slime-js-eval "$('.modal').find('.close').click(); mytop = $('body').scrollTop(); SammyApp.refresh(); $('body').scrollTop(mytop)") ;; make this a hook?
